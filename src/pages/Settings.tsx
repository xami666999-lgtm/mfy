...
import { Save, Check, ExternalLink, RefreshCw, FolderPlus, UserPlus } from 'lucide-react'
        ...
        <Section title="General">
          ...
          <div className="pt-1">
            <button
              ...
              onClick={async () => {
                const api = (window as any).electronAPI
                if (api?.resetSetup) await api.resetSetup()
                store.setSetupComplete(false)
                setResetMsg(true)
                setTimeout(() => setResetMsg(false), 4000)
              }}
              className="h-8 px-3 rounded-lg bg-red-500/10 border border-red-500/30 text-xs text-red-400 hover:bg-red-500/20 transition-all"
            >
              Reset Setup Wizard
            </button>
            {resetMsg && <p className="text-[10px] text-white/40 mt-2">Setup flag cleared — the wizard will show again.</p>}
          </div>
        </Section>
      </div>
    </div>
