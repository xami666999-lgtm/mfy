...
old_franchises = '''{(franchisesData.length > 0 || loadingFranchises) && (
          <section style={{ marginTop: 16 }}>
            ...
            {!loadingFranchises && franchisesData.length === 0 && tmdbApiKey && (
...
new_franchises = '''{(franchisesData.length > 0 || loadingFranchises) && (
          <section style={{ marginTop: 16 }}>
            <div style={{ 
              display: 'flex', 
              justifyContent: 'space-between', 
                    ...
                    <div style={{ display: 'flex', gap: 8, paddingBottom: 8 }}>
                      {Array.from({ length: 4 }).map((_, j) => (
                        <div key={j} style={{ flexShrink: 0, width: 120 }}>
                          <div className="aspect-[2/3] bg-white/[0.05] animate-pulse rounded-lg mb-2" />
                          <div className="h-3 w-full bg-white/[0.05] animate-pulse rounded" />
                          <div className="h-2 w-3/4 bg-white/[0.03] animate-pulse rounded mt_1" />
                        </div>
                      ))}
                    </div>
                  </div>
...
content = content.replace(old_franchises, new_franchises)

with open('C:\\Users\\Noah\\Desktop\\mfy-app\\src\\pages\\Board.tsx', 'w') as f:
    f.write(content)

print("Fixed franchises section")