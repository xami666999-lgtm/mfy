...
# Fix airing section - restructure completely
old_airing = '''        {(airingAnime.length > 0 || airingTVShows.length > 0 || loadingAiring) && (
...
new_airing = '''        {(airingAnime.length > 0 || airingTVShows.length > 0 || loadingAiring) && (
          <section style={{ marginTop: 16 }}>
            <div style={{ 
              display: 'flex', 
              justifyContent: 'space-between', 
...
content = content.replace(old_airing, new_airing)

with open('C:\\Users\\Noah\\Desktop\\mfy-app\\src\\pages\\Board.tsx', 'w') as f:
    f.write(content)

print("Fixed airing section")